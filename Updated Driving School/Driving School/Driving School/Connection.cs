...
namespace Driving_School
{
    class Connection
    {
        ...
        public Boolean cancelBooking(int booking_id, int package_id, int client_id)
        {
            string query = "SELECT number_of_lessons FROM Packages WHERE package_id='" + package_id + "'";
            com = new SqlCommand(query, con);
            da = new SqlDataAdapter(com);
            dt = new DataTable();
            da.Fill(dt);
            int lessons = dt.Rows[0].Field<int>("number_of_lessons");

            query = "UPDATE Booking SET lesson_counter=@count WHERE booking_id=@id";
            com = new SqlCommand(query, con);
            com.Parameters.AddWithValue("@count", lessons);
